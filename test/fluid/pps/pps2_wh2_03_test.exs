defmodule Fluid.Pps.Pps2Wh203Test do
  @moduledoc """
  ...
  describe "pps = 1 , wh = 2 , pps_type = indeterminate, " do
    setup do
      ###### setup warehouses for circularity  ######
      warehouses = Factory.setup_warehouses_for_circularity(:mix_det_indet)

      # warehouse 2
      warehouse_2 = Enum.at(warehouses, 1)

      # warehouse 3
      warehouse_3 = Enum.at(warehouses, 2)
      ...
      [cp_10, cp_13] = warehouse_3.capped_pools
      [fp_11, fp_12] = warehouse_3.fixed_pools

      [ct_14, ct_15, _ct_16, ct_17] = warehouse_3.capped_tanks
      ...
      updated_warehouses =
        warehouses
        |> List.replace_at(1, warehouse_2)
        |> List.replace_at(2, warehouse_3)
    ...
    test "pps.type = indeterminate - NEGATIVE ", %{
    ...
    test "pps.type = indeterminate - POSITIVE ", %{
      pps_analysis_map: pps_analysis_map,
      circularity_analysis: %{determinate: _determinate, indeterminate: indeterminate}
    } do
      ...
      |> Enum.map(fn
        %{type: :indet_pps_only, related_wh: wh_list} ->
          assert Enum.count(wh_list, fn wh -> Map.has_key?(indeterminate, wh.id) end) >= 1
      ...
      assert Enum.count(indet_pps_list) == 2
    end
    ...
    test "pools forming pps - ", %{
      ...
      assert forms_pps_either?([cp_1, cp_10, fp_11], indet_pps_list)

      # [cp_2, fp_2, fp_12, cp_13] form a pps (either pps_1 or pps_2)
      assert forms_pps_either?([cp_2, fp_2, fp_12, cp_13], indet_pps_list)
    end
  end
end
